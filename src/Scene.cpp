...
Scene::Scene(const char* xmlPath)
{
    ...
    while (camElement != NULL)
    {
        ...
        str = camFieldElement->GetText();
        sscanf(str, "%lf %lf %lf %lf %lf %lf %d %d", &camera->left, &camera->right, &camera->bottom, &camera->top,
...
Matrix4 createOrthographicProjectionMatrix(Camera* camera)
{
    double orthoMatrixValues[4][4] = {
        { 2.0 / (camera->right - camera->left), 0,                                    0,
         -(camera->right + camera->left) / (camera->right - camera->left)                                                                                                               },
        { 0,                                    2.0 / (camera->top - camera->bottom), 0,
...
Matrix4 createPerspectiveProjectionMatrix(Camera* camera)
{
    double perspectiveMatrixValues[4][4] = {
        { (2.0 * camera->near) / (camera->right - camera->left), 0,
         (camera->right + camera->left) / (camera->right - camera->left),                                                                                                             0 },
        { 0,                                                     (2.0 * camera->near) / (camera->top - camera->bottom),
         (camera->top + camera->bottom) / (camera->top - camera->bottom),                                                                                                             0 },
    ...
    for (int i = 0; i < mesh->numberOfTransformations; i++)
    {
        ...
        case 'r' : {
            ...
            double rotationMatrixValues[4][4] = {
                { rotation->ux * rotation->ux * (1 - cos(angle)) + cos(angle),
                 rotation->ux * rotation->uy * (1 - cos(angle)) - rotation->uz * sin(angle),
                 rotation->ux * rotation->uz * (1 - cos(angle)) + rotation->uy * sin(angle),        0 },
                { rotation->uy * rotation->ux * (1 - cos(angle)) + rotation->uz * sin(angle),
                 rotation->uy * rotation->uy * (1 - cos(angle)) + cos(angle),
                 rotation->uy * rotation->uz * (1 - cos(angle)) - rotation->ux * sin(angle),        0 },
                { rotation->uz * rotation->ux * (1 - cos(angle)) - rotation->uy * sin(angle),
                 rotation->uz * rotation->uy * (1 - cos(angle)) + rotation->ux * sin(angle),
                 rotation->uz * rotation->uz * (1 - cos(angle)) + cos(angle),                       0 },
                { 0,                                                                          0, 0, 1 }
            };
...
bool Scene::liangBarskyClip(Camera* camera, Vec4& p0, Vec4& p1)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        if (p[i] > 0)
        {
            ...
            if (t > t0)
                t0 = t;   // Line enters clipping area
        }
        else if (p[i] < 0)
        {
            double t = q[i] / p[i];
...
void Scene::forwardRenderingPipeline(Camera* camera)
{
    ...
    Matrix4 projectionMatrix = (camera->projectionType == 0) ? createOrthographicProjectionMatrix(camera) :
    ...
    for (Mesh* mesh : this->meshes)
    {
        ...
        for (int i = 0; i < mesh->triangles.size(); ++i)
        {
            ...
            if (cullingEnabled && isTriangleBackFacing(v0Homogeneous, v1Homogeneous, v2Homogeneous))
                continue;
            ...
            if (mesh->type == 0)   // wireframe
            {
                ...
                if (edgeTwoVisible)
                {
                    rasterizeLine(v1tempHomogeneous, v2Homogeneous, c1, c2, this->image, camera->horRes,
                                  camera->verRes);
                }
                if (edgeThreeVisible)
                {
                    rasterizeLine(v2tempHomogeneous, v0tempHomogeneous, c2, c0, this->image, camera->horRes,
                                  camera->verRes);
                }
            }
            else   // solid
